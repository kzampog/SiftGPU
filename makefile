...
INSTALL_INCLUDE_PATH=/usr/local/include/siftgpu

install: 
	mkdir -p $(INSTALL_LIB_PATH)
	cp $(BIN_DIR)/libsiftgpu.so $(INSTALL_LIB_PATH)
	mkdir -p $(INSTALL_INCLUDE_PATH)
	cp ${SRC_SIFTGPU}/SiftGPU.h $(INSTALL_INCLUDE_PATH)

uninstall:
	rm -r $(INSTALL_INCLUDE_PATH)
	rm $(INSTALL_LIB_PATH)/libsiftgpu.so
